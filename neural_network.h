...
class NeuralNetwork{
    ...
    void initialize(size_t input_dim, size_t output_dim, const std::vector<size_t>& hidden_layer_dims);
    ...
    size_t num_hidden_layers() const {
        return num_layers() - 2u;
    }

    size_t num_non_input_layers() const {
        return num_layers() - 1u;
    }
    ...
    std::function<double(double)> activation_func_ = [](const double x) { return std::abs(x); };
};
